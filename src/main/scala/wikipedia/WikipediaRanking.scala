...
object WikipediaRanking {
  ...
  val conf: SparkConf  = new SparkConf().setMaster("local").setAppName("spark-wikipedia")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate(0)((u, article) => (u + (if (article.mentionsLanguage(lang)) 1 else 0)), (_ + _))
//    rdd.filter(_.mentionsLanguage(lang)).map(x => (x, 1)).count().toInt
